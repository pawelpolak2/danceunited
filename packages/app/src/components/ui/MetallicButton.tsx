...
export function MetallicButton({ children, className = '', ...props }: MetallicButtonProps) {
  return (
    <button
      className={`metallic-button rounded-lg px-6 py-2.5 font-bold hover:scale-[1.02] hover:shadow-amber-500/20 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50 ${className}
      `}
      {...props}
    >
